...
public class ProviderForProduceGenerator extends BaseGenerator {
    ...
    public void validate() {
        if (element.getKind() != ElementKind.METHOD) {
            throw new SimpleIocElementException("mlesiewski.simpleioc.Produce is only applicable for methods", element);
        }
        Set<Modifier> modifiers = element.getModifiers();
        if (modifiers.contains(Modifier.ABSTRACT)) {
            throw new SimpleIocElementException("mlesiewski.simpleioc.Produce is only applicable for non-abstract methods", element);
        }
        if (!modifiers.contains(Modifier.PUBLIC)) {
            throw new SimpleIocElementException("mlesiewski.simpleioc.Produce is only applicable public methods", element);
        }
        if (modifiers.contains(Modifier.STATIC)) {
            throw new SimpleIocElementException("mlesiewski.simpleioc.Produce is only applicable for non-static methods", element);
        }
    }
