...
class Template {
     ...
     *
     * @throws SimpleIocAptException if an empty key {{}} is found in template text
     */
    Template(String text) {
        this.text = text;
        boolean emptyKey = text.contains("{{}}");
        if (emptyKey) {
            throw new SimpleIocAptException("empty key {{}} in template");
        }
     ...
     * @throws SimpleIocAptException on unused key in template or in inputValues
     */
    String compile(Map<String, String> inputValues) {
        if (keyCount != inputValues.size()) {
            throw new SimpleIocAptException("template has " + keyCount + " keys but " + inputValues.size() + " were provided");
        }
        String result = text;
